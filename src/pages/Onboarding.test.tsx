...
import { OnboardingContext, OnboardingFields, signupReducer } from './Onboarding'
...
const Page = ({ initialState = defaultOnboardingState }: { initialState?: OnboardingFields }) => {
  ...
  return (
    ...
    >
      <Routes>
        ...
        <Route path="grants" element={<Navigate to={`/grants/0`} replace={true} />} />
...
describe('Onboarding', () => {
  it('should allow configuring user details with validation', async () => {
    ...
    expect(emailField).toHaveValue('invalidemail')
    ...
    // Blur the email field again
    await userEvent.tab()

    // Check if the error message is no longer displayed
    expect(screen.queryByText('Invalid email')).toBeNull()

    const nextButton = screen.getByRole('button', { name: 'Next' })
    await userEvent.click(nextButton)

    // Check if the name field is no longer displayed
    expect(screen.queryByRole('textbox', { name: /who is setting/ })).not.toBeInTheDocument()
  })
  ...
  it('should allow configuring shareholders', async () => {
    ...
    const addShareholdersButton = screen.getByRole('button', {
      name: /Add Shareholder/, // error still not resolved
    })
