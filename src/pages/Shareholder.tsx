...
export function ShareholderPage() {
  ...
  const grantQuery = useQuery<{ [dataID: number]: Grant }>('grants', () => fetch('/grants').then((e) => e.json()))

  const shareholderQuery = useQuery<{ [dataID: number]: Shareholder }>('shareholders', () =>
  ...
  const companyQuery = useQuery<Company>('company', () => fetch('/company').then((e) => e.json()))
  ...
  const grantMutation = useMutation<Grant, unknown, Omit<Grant, 'id'>>(
      ...
      }).then((res) => res.json()),
    {
      onSuccess: (data) => {
        ...
        queryClient.setQueriesData<{ [id: number]: Grant } | undefined>('grants', (g) => {
          if (g) {
        ...
        // ANSWER: By calling invalidateQueries with the appropriate query keys,
        // to ensure the queries are invalidated and refetched, triggering an instant re-render
        // on the components consuming the updated data.
        queryClient.invalidateQueries('shareholders')
        queryClient.invalidateQueries('grants')
      },
    }
  )
  // set initial value per share
  const [valuePerShare, setValuePerShare] = React.useState<number>(0)
  React.useEffect(() => {
    if (companyQuery.data) {
      const initialValue = companyQuery.data.shareValues[draftGrant.type] || 0
      setValuePerShare(initialValue)
    }
  }, [companyQuery.data, draftGrant.type])

  const companyMutation = useMutation<Company, unknown, Company>(
    (updatedCompany) => {
      console.log({ updatedCompany })
      return fetch(`/company/${companyQuery.data?.name}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function submitGrant(e: React.FormEvent) {
    ...
    try {
      ...
      const updatedCompany = produce<Company>(companyQuery.data ?? ({} as Company), (draft) => {
        draft.shareValues[draftGrant.type] = valuePerShare
      })
      await companyMutation.mutateAsync(updatedCompany)
    } catch (e) {
      console.warn(e)
    }
  }

  if (grantQuery.status !== 'success' || shareholderQuery.status !== 'success') {
    return <Spinner />
  }
  ...
  return (
    <Stack>
      ...
      <Stack direction="row" spacing="8">
        ...
        <Stack>
          ...
          <Text fontSize="sm" fontWeight="thin">
            <strong data-testid="grants-issued">{shareholder.grants.length}</strong> grants issued
          </Text>
          <Text fontSize="sm" fontWeight="thin">
            <strong data-testid="shares-granted">
              {shareholder.grants.reduce((acc, grantID) => acc + grantQuery.data[grantID].amount, 0)}
            </strong>{' '}
            shares
          </Text>
        </Stack>
      ...
      <Table size="s">
        ...
        <Tbody role="rowgroup">
          {shareholderQuery.data[parseInt(shareholderID, 10)].grants.map((grantID) => {
            const { name, issued, amount, type } = grantQuery.data[grantID]
            const shareValue = companyQuery.data?.shareValues[type]
            const totalValue = shareValue !== undefined ? shareValue * amount : 0
            return (
              <Tr key={grantID}>
                <Td>{name}</Td>
