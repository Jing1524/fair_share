...
describe('ShareholderPage', () => {
  it('should show a sumamry of shares', async () => {
    ...
    render(
      <Router>
        <Routes>
          <Route path="/shareholder/:shareholderID" element={<ShareholderPage />} />
        </Routes>
      </Router>,
  ...
  it('should allow adding new grants', async () => {
    ...
    expect(await within(grantTable).findByText(/Incentive Package 2019/)).toBeInTheDocument()
    expect(within(grantTable).getByText(/2000/)).toBeInTheDocument()
    expect(within(grantTable).getByText(new Date('2010-12-12').toLocaleDateString())).toBeInTheDocument()
  })
})
